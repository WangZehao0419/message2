...
function checklogin_name() {
    ...
    $("#login_name").html("");
    if (a == "") {
        $("#login_name").html("不能为空").css("color", "red");
        ok1 = false;
    } else if (!/[a-z]|[A-Z]{1}/.test(b)) {
        $("#login_name").html("必须以字母开头").css("color", "red");
        ok1 = false;
    } else if (a.length < 6) {
        $("#login_name").html("长度最少6位").css("color", "red");
        ok1 = false;
    } else if (a.length > 15) {
        $("#login_name").html("长度最多15位").css("color", "red");
        ok1 = false;
    } else {
        ...
        $.post("/glory/rest/getUSerByName", fromVals, function (data, state) {
                //这里显示从服务器返回的数据
                if (data.flag == 1) {
                    $("#login_name").html("登录名已存在").css("color", "red");
                    ok1 = false;
                } else {
                    $("#login_name").html("√").css("color", "green");
                    ok1 = true;
                }
            },
            "json"
...
function checkpassword() {
    ...
    if (a == "") {
        ...
        $("#password").html("长度最少6位").css("color", "red");
        ok2 = false;
    } else if (a.length > 16) {
        $("#password").html("长度最多15位").css("color", "red");
        ok2 = false;
    } else {
...
function checkpassword2() {
    ...
    $("#password2").html("");
    if (a == "") {
        $("#password2").html("不能为空").css("color", "red");
        ok3 = false;
    } else if (a.length < 6) {
        $("#password2").html("长度最少6位").css("color", "red");
        ok3 = false;
    } else if (a.length > 16) {
        $("#password2").html("长度最多15位").css("color", "red");
        ok3 = false;
    } else if ($("#p1").val() != $("#p2").val()) {
        $("#password2").html("两次输入密码不一致").css("color", "red");
        ok3 = false;
    } else {
        $("#password2").html("√").css("color", "green");
        ok3 = true;
    }
}
...
function checkusername() {
    ...
    $("#username").html("");
    if (a == "") {
        $("#username").html("不能为空").css("color", "red");
        ok4 = false;
    } else if (a.length < 2) {
        $("#username").html("长度最少2位").css("color", "red");
        ok4 = false;
    } else if (a.length > 11) {
        $("#username").html("长度最多10位").css("color", "red");
        ok4 = false;
    } else {
        $("#username").html("√").css("color", "green");
        ok4 = true;
    }
}
