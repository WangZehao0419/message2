...
$("input[name=newpwd]").blur(function () {
    ...
    $("#newpwd").html("");
    if (a == "") {
        $("#newpwd").html("不能为空").css("color", "red");
        ok6 = false;
    } else if (a.length < 6) {
        $("#newpwd").html("长度最少6位").css("color", "red");
        ok6 = false;
    } else if ($("input[name=newpwd1]").val() != "") {
        if ($("input[name=newpwd]").val() != $("input[name=newpwd1]").val()) {
            $("#newpwd1").html("两次密码不一致").css("color", "red");
            ok6 = false;
        } else {
            ok6 = true;
        }
...
$("input[name=newpwd1]").blur(function () {
    ...
    $("#newpwd1").html("");
    if (a == "") {
        $("#newpwd1").html("不能为空").css("color", "red");
        ok7 = false;
    } else if (a.length < 6) {
        $("#newpwd1").html("长度最少6位").css("color", "red");
        ok7 = false;
