...
$("input[name=account]").blur(function () {
    ...
    $("#account").html("");
    if (a == "") {
        $("#account").html("不能为空").css("color", "red");
        ok1 = false;
    } else if (!/[a-z]|[A-Z]{1}/.test(b)) {
        $("#account").html("必须以字母开头").css("color", "red");
        ok1 = false;
    } else if (a.length < 6) {
        $("#account").html("长度最少6位").css("color", "red");
        ok1 = false;
    } else {
        $.ajax({
            ...
            success: function (res) {
                if (res == '0') {
                    ...
                    $("#account").html("帐号已存在").css("color", "red");
                    ok1 = false;
                }
            }
        })
...
$("input[name=pwd]").blur(function () {
    ...
    $("#pwd").html("");
    if (a == "") {
        $("#pwd").html("不能为空").css("color", "red");
        ok2 = false;
    } else if (a.length < 6) {
        $("#pwd").html("长度最少6位").css("color", "red");
        ok2 = false;
    } else if ($("input[name=pwd1]").val() != "") {
        if ($("input[name=pwd]").val() != $("input[name=pwd1]").val()) {
            $("#pwd1").html("两次密码不一致").css("color", "red");
            ok2 = false;
        } else {
            ok2 = true;
        }
...
$("input[name=pwd1]").blur(function () {
    ...
    $("#pwd1").html("");
    if (a == "") {
        $("#pwd1").html("不能为空").css("color", "red");
        ok3 = false;
    } else if (a.length < 6) {
        $("#pwd1").html("长度最少6位").css("color", "red");
        ok3 = false;
...
$("input[name=name]").blur(function () {
    ...
    if (a == "") {
        ...
        $("#name").html("长度最少2位").css("color", "red");
        ok4 = false;
    } else {
        ok4 = true;
    }
...
$("input[name=company]").blur(function () {
    ...
    $("#company").html("");
    if (a == "") {
        $("#company").html("不能为空").css("color", "red");
        ok8 = false;
    } else if (a.length < 2) {
        $("#company").html("长度最少2位").css("color", "red");
        ok8 = false;
    } else {
        ok8 = true;
    }
...
$("input[name=idcard]").blur(function () {
    ...
    $("#idcard").html("");
    if (a == "") {
        $("#idcard").html("不能为空").css("color", "red");
        ok5 = false;
    } else if (!/^[1-9]{1}[0-9]{14}$|^[1-9]{1}[0-9]{16}([0-9]|[xX])$/.test(a)) {
        $("#idcard").html("身份证格式有误").css("color", "red");
        ok5 = false;
    } else {
        ok5 = true;
    }
...
$("input[name=email]").blur(function () {
    ...
    $("#email").html("");
    if (a == "") {
        $("#email").html("不能为空").css("color", "red");
        ok6 = false;
    } else if (!/(^[a-zA-Z0-9_-]+@[a-zA-Z0-9_-]+\.[a-zA-Z0-9_-]+$)|(^$)/.test(a)) {
        $("#email").html("邮箱格式有误").css("color", "red");
        ok6 = false;
    } else {
        ok6 = true;
    }
...
$("input[name=phone]").blur(function () {
    ...
    if (a == "") {
        ...
        $("#phone").html("手机号格式有误").css("color", "red");
        ok7 = false;
    } else {
        ok7 = true;
    }
